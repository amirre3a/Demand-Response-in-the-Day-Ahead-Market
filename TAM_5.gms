...
obj..                    OF=e=sum(t, sum(bus, VOLL*PL(bus,t)  ) + sum(bus$el(bus),MUU(bus,'MU')*Del(bus,t)) - sum(g,gendata(g,'MC')*Pg(g,t))
                         - sum((bus,sm)$c(bus),-bid(bus,sm)*Pc(bus,t)) - sum((bus,sm)$df(bus),bid2(bus,sm)*Pdf(bus,t))    ) ;
...
eq6(bus,t)..           DL(bus,t) =e= PL(bus,t) + Del(bus,t)$el(bus) + Pc(bus,t)$c(bus) + Pdf(bus,t)$df(bus);
