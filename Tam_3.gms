...
obj..                    OF=e=sum(t,sum(g,gendata(g,'MC')*Pg(g,t)))+sum((bus,sm,t)$c(bus),-bid(bus,sm)*Pc(bus,t)) + sum((bus,sm,t)$df(bus),bid2(bus,sm)*Pdf(bus,t));
...
eq6(bus,t)..           DL(bus,t) =e= PL(bus,t) + Pc(bus,t)$c(bus) + Pdf(bus,t)$df(bus);

eq7(bus,t)..           sum(g$GB(bus,g), pg(g,t)) - DL(bus,t) =e= sum(node$conex(node,bus),Pij(bus,node,t) );

eq8(bus,t)..           Phs(bus,t) =l= PL(bus,t);

eq9(bus,t)$c(bus)..            -1*PL(bus,t) =l= Pc(bus,t);

eq10(bus,t)$c(bus)..           Pc(bus,t) =l= 0;

eq11(bus,t)$df(bus)..          -1*PL(bus,t) =l= Pdf(bus,t);

eq12(bus,t)$df(bus)..          Pdf(bus,t) =l= 0;
